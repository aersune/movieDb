...
class RecommendTvSeries extends StatelessWidget {
  const RecommendTvSeries({
    super.key,
              ...
              if(state is TvSeriesDetailsLoadedState){
                return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      return InkWell(
                        onTap: (){
                          context.read<MoviesProvider>().firstDetailsId = state.recommendSeries!.results![index].id!;
                          context.read<MoviesDbBloc>().add(TvSeriesDetailsEvent(idMovie: state.recommendSeries!.results![index].id ?? 0));
